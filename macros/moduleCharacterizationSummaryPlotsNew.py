...
parser.add_argument("-c",  "--corrections",   required=True, type=str, help="comma-separated list of corrections: totRatioCorr, totRatioPhaseCorr, posCorr, posPhaseCorr...")
...
# INPUT
inputdir = '/afs/cern.ch/work/f/fcetorel/private/work2/dev_TB_CERN_Sept2023/plots/TOFHIR2C/onlyPosCorr/'
#source = 'Laser'
source = 'TB'
...
os.system('mkdir %s/summaryPlotsNew/'%outdir)
os.system('mkdir %s/summaryPlotsNew/tot/'%outdir)
os.system('mkdir %s/summaryPlotsNew/energy/'%outdir)
os.system('mkdir %s/summaryPlotsNew/timeResolution/'%outdir)
os.system('mkdir %s/summaryPlotsNew/timeResolution/fits/'%outdir)
...
g_deltaT_bestTh_vs_vov_average = {} # g [energyBin]
  ...
  for l in ['L','R','L-R']:
     for vov in Vovs:
        for thr in thresholds:
           ...
           if (l=='L-R'):
              for enBin in enBins: 
                 g_deltaT_vs_bar[corr,vov, thr, enBin] = ROOT.TGraphErrors()
 
  for bar in bars:
     for enBin in enBins: 
        g_deltaT_bestTh_vs_vov[corr, bar, enBin] = ROOT.TGraphErrors()
        
  for vov in Vovs:
      for enBin in enBins: 
          g_deltaT_bestTh_vs_bar[corr, vov, enBin] = ROOT.TGraphErrors()
 
  for enBin in enBins: 
     g_deltaT_bestTh_vs_vov_average[corr, enBin] = ROOT.TGraphErrors()
...
for label in label_list:
   ...
   for corr in corrections:                        
      ...
      for bar in bars:
         for vov in Vovs:
            for enBin in enBins:    
               bestRes[corr, bar, vov, enBin] = [9999, 9999]
         ...
         for vov in Vovs:
            ...
            for thr in thresholds: 
               ...
               for enBin in enBins:
                  ...
                  #ctemp.SaveAs(outdir+'/summaryPlots/timeResolution/fits/'+'/c_h1_deltaT_%s_bar%02dL-R_Vov%.02f_th%02d_energyBin%02d.png'%(corr, bar, vov, thr, enBin))
   
                  # graphs vs threshold
                  g_deltaT_vs_th[corr, bar, vov, enBin].SetPoint(g_deltaT_vs_th[corr, bar, vov, enBin].GetN(), thr, tRes[corr, enBin][0]/kscale )
                  g_deltaT_vs_th[corr, bar, vov, enBin].SetPointError(g_deltaT_vs_th[corr, bar, vov, enBin].GetN()-1, 0, tRes[corr, enBin][1]/kscale)
   
                  # graphs vs Vov
                  g_deltaT_vs_vov[corr, bar, thr, enBin].SetPoint(g_deltaT_vs_vov[corr, bar, thr, enBin].GetN(), VovsEff[vov], tRes[corr, enBin][0]/kscale )
                  g_deltaT_vs_vov[corr, bar, thr, enBin].SetPointError(g_deltaT_vs_vov[corr, bar, thr, enBin].GetN()-1, 0, tRes[corr, enBin][1]/kscale)
   
                 
                  if (thr == thRef):
                     g_deltaT_vs_bar[corr, vov, thr, enBin].SetPoint(g_deltaT_vs_bar[corr, vov, thr, enBin].GetN(), bar, tRes[corr, enBin][0]/kscale )
                     g_deltaT_vs_bar[corr, vov, thr, enBin].SetPointError(g_deltaT_vs_bar[corr, vov, thr, enBin].GetN()-1, 0, tRes[corr,enBin][1]/kscale)
            ...
            for enBin in enBins:
               ...
               if (bestRes[corr, bar, vov, enBin][0]!= 9999):
                  g_deltaT_bestTh_vs_vov[corr, bar, enBin].SetPoint(g_deltaT_bestTh_vs_vov[corr, bar, enBin].GetN(), VovsEff[vov], (bestRes[corr, bar, vov, enBin][0])/kscale )
   ...
   for corr in corrections:
     for enBin in enBins:
        for vov in Vovs:
           fitpol0 = ROOT.TF1('fitpol0_'+corr,'pol0',-1,16) 
           g_deltaT_bestTh_vs_bar[corr, vov, enBin].Fit(fitpol0,'QSN')
           ave, err = [fitpol0.GetParameter(0),fitpol0.GetParError(0)]
           g_deltaT_bestTh_vs_vov_average[corr, enBin].SetPoint(g_deltaT_bestTh_vs_vov_average[corr, enBin].GetN(), VovsEff[vov], ave)
           g_deltaT_bestTh_vs_vov_average[corr, enBin].SetPointError(g_deltaT_bestTh_vs_vov_average[corr, enBin].GetN()-1, 0, err)
...
for bar in bars:
    ...
    leg.Draw()    
    latex.Draw('same')
    ctot1.SaveAs(outdir+'/summaryPlotsNew/tot/'+ctot1.GetName()+'.png')
    ctot1.SaveAs(outdir+'/summaryPlotsNew/tot/'+ctot1.GetName()+'.pdf')
    hPad1.Delete()
    ...
    leg.Draw()    
    ctot2.SaveAs(outdir+'/summaryPlotsNew/tot/'+ctot2.GetName()+'.png')
    ctot2.SaveAs(outdir+'/summaryPlotsNew/tot/'+ctot2.GetName()+'.pdf')
    hPad2.Delete()
    ...
    leg.Draw()
    cen1.SaveAs(outdir+'/summaryPlotsNew/energy/'+cen1.GetName()+'.png')
    cen1.SaveAs(outdir+'/summaryPlotsNew/energy/'+cen1.GetName()+'.pdf')
    hPadEn1.Delete()
    ...
    leg.Draw()
    cen2.SaveAs(outdir+'/summaryPlotsNew/energy/'+cen2.GetName()+'.png')
    cen2.SaveAs(outdir+'/summaryPlotsNew/energy/'+cen2.GetName()+'.pdf') 
    hPadEn2.Delete()   

    for corr in corrections:
      # -- time resolution vs threshold
      for enBin in enBins:
          ctres1 = ROOT.TCanvas('c_tRes_%s_vs_th_bar%.02d_enBin%02d'%(corr, bar,enBin))
          ...
          for i, vov in enumerate(Vovs):
              ...
              g_deltaT_vs_th[corr, bar, vov, enBin].SetMarkerStyle(markers[corr])
              g_deltaT_vs_th[corr, bar, vov, enBin].SetMarkerColor(cols[vov])
              g_deltaT_vs_th[corr, bar, vov, enBin].SetLineColor(cols[vov])
              g_deltaT_vs_th[corr, bar, vov, enBin].Draw('plsame')
 


              leg.AddEntry(g_deltaT_vs_th[corr, bar, vov, enBin], 'V_{OV}^{eff} = %.02f V'%VovsEff[vov], 'PL')
              outfile.cd()
              g_deltaT_vs_th[corr, bar, vov, enBin].Write('g_deltaT_%s_vs_th_bar%02d_Vov%.02f_enBin%02d'%(corr, bar, vov, enBin))
          leg.Draw()
          latex.Draw('same')
          ctres1.SaveAs(outdir+'/summaryPlotsNew/timeResolution/'+ctres1.GetName()+'.png')
          ctres1.SaveAs(outdir+'/summaryPlotsNew/timeResolution/'+ctres1.GetName()+'.pdf')
          hPadT1.Delete()


          # -- time resolution vs Vov
          ctres2 = ROOT.TCanvas('c_tRes_vs_Vov_bar%.02d_enBin%02d'%(bar,enBin))
          ...
          leg.SetFillStyle(0)
          for i, thr in enumerate(thresholds):
              g_deltaT_vs_vov[corr, bar, thr, enBin].SetMarkerStyle(markers[corr])
              g_deltaT_vs_vov[corr, bar, thr, enBin].SetMarkerColor(i+1)
              g_deltaT_vs_vov[corr, bar, thr, enBin].SetLineColor(i+1)
              g_deltaT_vs_vov[corr, bar, thr, enBin].Draw('plsame')
              
              leg.AddEntry(g_deltaT_vs_vov[corr, bar, thr, enBin], 'th. = %d'%thr, 'PL')
              outfile.cd()
              g_deltaT_vs_vov[corr, bar,thr, enBin].Write('g_deltaT_%s_vs_vov_bar%02d_th%02d_enBin%02d'%(corr, bar, thr, enBin)) 
          leg.Draw()
          latex.Draw('same')
          ctres2.SaveAs(outdir+'/summaryPlotsNew/timeResolution/'+ctres2.GetName()+'.png')
          ctres2.SaveAs(outdir+'/summaryPlotsNew/timeResolution/'+ctres2.GetName()+'.pdf')
          hPadT2.Delete()   

          # -- time resolution vs Vov at the best Th
          ctres2 = ROOT.TCanvas('c_tRes_bestTh_vs_Vov_bar%.02d_enBin%02d'%(bar,enBin))
          hPadT2 = ROOT.TH2F('hPadT2','', 6, 0., vovMax,10, tResMin,tResMax)
          hPadT2.SetTitle(";V_{OV}^{eff} [V];#sigma_{t} [ps]")
          hPadT2.Draw()
          ctres2.SetGridy()
          g_deltaT_bestTh_vs_vov[corr, bar, enBin].SetMarkerStyle(markers[corr])
          g_deltaT_bestTh_vs_vov[corr, bar, enBin].SetMarkerColor(1)
          g_deltaT_bestTh_vs_vov[corr, bar, enBin].SetLineColor(1)
          g_deltaT_bestTh_vs_vov[corr, bar, enBin].Draw('plsame')
          latex.Draw('same')
          outfile.cd() 
          g_deltaT_bestTh_vs_vov[corr, bar, enBin].Write('g_deltaT_%s_bestTh_vs_vov_bar%02d_enBin%02d'%(corr, bar, enBin))
          ctres2.SaveAs(outdir+'/summaryPlotsNew/timeResolution/'+ctres2.GetName()+'.png')
          ctres2.SaveAs(outdir+'/summaryPlotsNew/timeResolution/'+ctres2.GetName()+'.pdf')
          hPadT2.Delete()   
...
for enBin in enBins:
   ctres2 = ROOT.TCanvas('c_tRes_bestTh_vs_Vov_enBin%02d_average'%(enBin))
   hPadT2 = ROOT.TH2F('hPadT2','', 6, 0.0, vovMax,10, tResMin,tResMax)
   hPadT2.SetTitle(";V_{OV}^{eff} [V];#sigma_{t} [ps]")
   hPadT2.Draw()
   ctres2.SetGridy()
   g_deltaT_bestTh_vs_vov_average[corr, enBin].SetMarkerStyle(markers[corr])
   g_deltaT_bestTh_vs_vov_average[corr, enBin].SetMarkerColor(1)
   g_deltaT_bestTh_vs_vov_average[corr, enBin].SetLineColor(1)
   g_deltaT_bestTh_vs_vov_average[corr, enBin].Draw('plsame')
   latex.Draw('same')
   outfile.cd() 
   g_deltaT_bestTh_vs_vov_average[corr, enBin].Write('g_deltaT_%s_bestTh_vs_vov_enBin%02d_average'%(corr, enBin))
   ctres2.SaveAs(outdir+'/summaryPlotsNew/timeResolution/'+ctres2.GetName()+'.png')
   ctres2.SaveAs(outdir+'/summaryPlotsNew/timeResolution/'+ctres2.GetName()+'.pdf')
   hPadT2.Delete()   
...
for i, vov in enumerate(Vovs):
   ...
   leg.Draw()
   ctot3.SaveAs(outdir+'/summaryPlotsNew/tot/'+ctot3.GetName()+'.png')
   ctot3.SaveAs(outdir+'/summaryPlotsNew/tot/'+ctot3.GetName()+'.pdf')    
   hPad3.Delete()
...
for i, vov in enumerate(Vovs):
   ...
   leg.Draw()
   cen3.SaveAs(outdir+'/summaryPlotsNew/energy/'+cen3.GetName()+'.png')
   cen3.SaveAs(outdir+'/summaryPlotsNew/energy/'+cen3.GetName()+'.pdf')    
   hPadEn3.Delete()
...
for corr in corrections:
  for i, vov in enumerate(Vovs):
     for enBin in enBins:
        ctres3 = ROOT.TCanvas('c_%s_refTh_vs_bar_Vov%.02f_enBin%02d'%(corr, vov,enBin))
        ...
        leg.SetBorderSize(0)
        leg.SetFillStyle(0)
        g_deltaT_vs_bar[corr, vov, thRef, enBin].SetMarkerStyle(markers[corr])
        g_deltaT_vs_bar[corr, vov, thRef, enBin].SetMarkerColor(cols[vov])
        g_deltaT_vs_bar[corr, vov, thRef, enBin].SetLineColor(cols[vov])
        g_deltaT_vs_bar[corr, vov, thRef, enBin].Draw('psame')
        outfile.cd() 
        g_deltaT_vs_bar[corr, vov, thRef, enBin].Write('g_deltaT_%s_vs_bar__Vov%.02f_th%02d'%(corr, vov,thRef))
        leg.AddEntry(g_deltaT_vs_bar[corr, vov, thRef, enBin], 'V_{OV}^{eff} = %.02f V'%VovsEff[vov], 'PL')
     leg.Draw()
     latex.Draw('same')
     ctres3.SaveAs(outdir+'/summaryPlotsNew/timeResolution/'+ctres3.GetName()+'.png')
     ctres3.SaveAs(outdir+'/summaryPlotsNew/timeResolution/'+ctres3.GetName()+'.pdf')    
     hPadT3.Delete()  

# -- time resolution vs bar at the best threshold
for corr in corrections:
  for enBin in enBins:
     ctres3 = ROOT.TCanvas('c_tRes_%s_bestTh_vs_bar_enBin%02d'%(corr, enBin))
     hPadT3 = ROOT.TH2F('hPadT3','', 100, -0.5, 15.5,100, tResMin,tResMax)
     hPadT3.SetTitle("; bar; #sigma_{t} [ps]")
     hPadT3.Draw()
     ctres3.SetGridy()
     ...
     for i, vov in enumerate(Vovs):
        g_deltaT_bestTh_vs_bar[corr, vov, enBin].SetMarkerStyle(markers[corr])
        g_deltaT_bestTh_vs_bar[corr, vov, enBin].SetMarkerColor(cols[vov])
        g_deltaT_bestTh_vs_bar[corr, vov, enBin].SetLineColor(cols[vov])
