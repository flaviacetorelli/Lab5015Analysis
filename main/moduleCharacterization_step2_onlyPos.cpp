...
int main(int argc, char** argv)
{
  ...
  system(Form("mkdir -p %s/CTR_posCorr/",plotDir.c_str()));
  system(Form("mkdir -p %s/CTR_posCorr_phaseCorr/",plotDir.c_str()));
  ...
  // -- phase corr
  std::map<double,TProfile*> p1_deltaT_posCorr_vs_t1fineMean;
  ...
  std::map<double,TH2F*> h2_deltaT_posCorr_vs_t1fineMean;
 
  // -- position corr 
  std::map<double,TProfile*> p1_deltaT_vs_pos;
  ...
  std::map<double,TH1F*> h1_deltaT_posCorr;
  std::map<double,TH1F*> h1_deltaT_posPhaseCorr;
	  ...
	  if( h1_deltaT[index2] == NULL )
	    {
	      ...
	      p1_deltaT_vs_pos[index2] = new TProfile(Form("p1_deltaT_vs_pos_%s",labelLR_energyBin.c_str()),"",100,-50, 50);
	  ...
	  if( ( deltaT > timeLow ) && ( deltaT < timeHig ) )
	    {
	      ...
	      if (useTrackInfo && anEvent->nhits > 0 && anEvent->x > -100) {
                p1_deltaT_vs_pos[index2] -> Fill( anEvent->x,deltaT );
	      }
  ...
  //------------------
  //--- draw 3rd plots
  std::map<double,TF1*> fitFunc_posCorr;
	...
	for(int iEnergyBin = 1; iEnergyBin <= nEnergyBins; ++iEnergyBin)
	  {
	    ...
	    // -- draw deltaT vs pos
            if(!p1_deltaT_vs_pos[index2]) continue;
	    
	    c = new TCanvas(Form("c_deltaT_vs_pos_%s",labelLR_energyBin.c_str()),Form("c_deltaT_vs_pos_%s",labelLR_energyBin.c_str()));
	    
	    prof = p1_deltaT_vs_pos[index2];
	    prof -> SetTitle(Form(";posX [mm];#Deltat [ps]"));
	    prof -> GetYaxis() -> SetRangeUser(CTRMeans[index2]-3.*CTRSigmas[index2],CTRMeans[index2]+3.*CTRSigmas[index2]);
	    prof -> Draw("");
	    ...
	    fitFunc_posCorr[index2] = new TF1(Form("fitFunc_posCorr_%s",labelLR_energyBin.c_str()),"pol1",-50,50);
            fitFunc_posCorr[index2] -> SetRange( prof -> GetMean()-2*prof->GetRMS(), prof -> GetMean()+2*prof->GetRMS());
	    prof -> Fit(fitFunc_posCorr[index2],"QRS+");
	    fitFunc_posCorr[index2] -> SetLineColor(kRed);
	    fitFunc_posCorr[index2] -> SetLineWidth(2);
	    fitFunc_posCorr[index2] -> Draw("same");
	    
	    c -> Print(Form("%s/positionCorr/c_deltaT_vs_pos__%s.png",plotDir.c_str(),labelLR_energyBin.c_str()));
	    c -> Print(Form("%s/positionCorr/c_deltaT_vs_pos__%s.pdf",plotDir.c_str(),labelLR_energyBin.c_str()));
	    delete c;
	    delete latex;
      ...
      for(int entry = 0; entry < nEntries; ++entry)
	{
	  ...
	  if( h1_deltaT_energyRatioCorr[index2] == NULL )
	    {
	      ...
	      h2_deltaT_energyRatioCorr_totRatioCorr_vs_t1fineMean[index2] = new TH2F(Form("h2_deltaT_energyRatioCorr_totRatioCorr_vs_t1fineMean_%s",labelLR_energyBin.c_str()),"",50,0,1000, 2000, -12000., 12000.);
              ...
              h1_deltaT_posCorr[index2] = new TH1F(Form("h1_deltaT_posCorr_%s",labelLR_energyBin.c_str()),"",2000,-12000.,12000.);
	      h2_deltaT_posCorr_vs_t1fineMean[index2] = new TH2F(Form("h2_deltaT_posCorr_vs_t1fineMean_%s",labelLR_energyBin.c_str()),"",50,0,1000, 2000, -12000., 12000.);
	      p1_deltaT_posCorr_vs_t1fineMean[index2] = new TProfile(Form("p1_deltaT_posCorr_vs_t1fineMean_%s",labelLR_energyBin.c_str()),"",50,0,1000);
	    }
	  ...
	  if (fabs(deltaT - totRatioCorr)<10000 ) {
	    ...
	    //}
          ...
          if (useTrackInfo && anEvent->nhits > 0 && anEvent->x > -100 && fitFunc_posCorr[index2] != NULL) { 
	    float posCorr = fitFunc_posCorr[index2]->Eval(anEvent->x) - fitFunc_posCorr[index2]->Eval( p1_deltaT_vs_pos[index2]->GetMean());
 	    if (fabs(deltaT - posCorr)<10000 ){
               
              //std::cout << "deltaT = " << deltaT << " posCorr = " <<  posCorr << "  deltaT  - posCorr =" <<  deltaT  - posCorr << std::endl;
              h1_deltaT_posCorr[index2] -> Fill( deltaT  - posCorr );
              p1_deltaT_posCorr_vs_t1fineMean[index2] -> Fill( t1fineMean, deltaT - posCorr );
	      h2_deltaT_posCorr_vs_t1fineMean[index2] -> Fill( t1fineMean, deltaT - posCorr );   
	             
             //std::cout << "# Entries in pos-corr deltaT for bar " << labelLR_energyBin.c_str() << " is " <<  h1_deltaT_posCorr[index2] -> GetEntries() << std::endl;
            }
          }
	  }
	}
	  ...
	  for(int iEnergyBin = 1; iEnergyBin <= nEnergyBins; ++iEnergyBin)
	    {
	      ...
	      // -- pos corr deltaT
	      c3 = new TCanvas(Form("c_deltaT_posCorr_%s",labelLR_energyBin.c_str()),Form("c_deltaT_posCorr_%s",labelLR_energyBin.c_str()));
	      
	      histo = h1_deltaT_posCorr[index2];
              //std::cout << "# Entries in pos-corr deltaT for bar " << labelLR_energyBin.c_str() << " is " <<  histo -> GetEntries() << std::endl;
	      histo -> SetLineWidth(2);
	      histo -> SetLineColor(kBlue);
	      histo -> SetMarkerColor(kBlue);
	      
	      fitFunc = new TF1(Form("fitFunc_posCorr_%s",labelLR_energyBin.c_str()),"gaus",-10000, 10000);
	      drawDeltaT(c3, histo, fitFunc, "pos-corrected", "posCorr", "");
	      ...
	      fitFunc = new TF1(Form("fitFunc_%s",labelLR_energyBin.c_str()),"gaus",-10000,10000);
              ...
              drawDeltaT(c3, histo, fitFunc, "", "raw", "same");
	      ...
	      c3 -> Print(Form("%s/CTR_posCorr/c_deltaT_posCorr__%s.pdf",plotDir.c_str(),labelLR_energyBin.c_str()));
	      c3 -> Print(Form("%s/CTR_posCorr/c_deltaT_posCorr__%s.png",plotDir.c_str(),labelLR_energyBin.c_str()));
	      delete c3;
	      ...
	      // -- draw deltaT posCorr vs t1fine
	      if(!p1_deltaT_posCorr_vs_t1fineMean[index2]) continue;
	      c = new TCanvas(Form("c_deltaT_posCorr__vs_t1fineMean_%s",labelLR_energyBin.c_str()),Form("c_deltaT_posCorr_vs_t1fineMean_%s",labelLR_energyBin.c_str()));
	      c -> SetGridy();
	      
	      h2 = h2_deltaT_posCorr_vs_t1fineMean[index2]; 
	      h2 -> GetYaxis()->SetRangeUser(h2 -> GetMean(2) -600., h2 -> GetMean(2)+600);
	      h2 -> SetTitle(Form(";t1fineMean;#Deltat [ps]"));
	      h2 -> Draw("colz");
	      prof = p1_deltaT_posCorr_vs_t1fineMean[index2];
	      //prof -> Draw("pl");
	      prof -> Draw("plsame");
	      ...
	      c -> Print(Form("%s/phaseCorr/c_deltaT_posCorr_vs_t1fineMean__%s.png",plotDir.c_str(),labelLR_energyBin.c_str()));
	      c -> Print(Form("%s/phaseCorr/c_deltaT_posCorr_vs_t1fineMean__%s.pdf",plotDir.c_str(),labelLR_energyBin.c_str()));
	      delete c;
	      delete latex;	      
      ...
      for(int entry = 0; entry < nEntries; ++entry)
	{
	  ...
	  if (fabs(deltaT - totRatioCorr)<10000 ) {
	    ...
	    if (useTrackInfo && anEvent->nhits>0 && anEvent->x>-100) p1_deltaT_totRatioCorr_vs_posX[index2] ->Fill( anEvent->x, deltaT  - totRatioCorr - t1fineCorr);
          ...
          // -- posCorr
          if (useTrackInfo && anEvent->nhits>0 && anEvent->x>-100 && fitFunc_posCorr[index2] != NULL && p1_deltaT_posCorr_vs_t1fineMean[index2] != NULL ) { 
	    
	    float posCorr = fitFunc_posCorr[index2]->Eval(anEvent->x) - fitFunc_posCorr[index2]->Eval( p1_deltaT_vs_pos[index2]->GetMean());
	    t1fineBin = p1_deltaT_posCorr_vs_t1fineMean[index2]->FindBin(t1fineMean);
	    t1fineBin2 = p1_deltaT_posCorr_vs_t1fineMean[index2]->FindBin(h1_t1fineMean[index2]->GetMean());
	    t1fineCorr = p1_deltaT_posCorr_vs_t1fineMean[index2]->GetBinContent(t1fineBin) - p1_deltaT_posCorr_vs_t1fineMean[index2]->GetBinContent( t1fineBin2 );
	    
	    if( h1_deltaT_posPhaseCorr[index2] == NULL )
	      {
	        std::string labelLR_energyBin(Form("bar%02dL-R_Vov%.2f_th%02d_energyBin%02d",anEvent->barID,anEvent->Vov,anEvent->vth1,energyBinAverage));
	        h1_deltaT_posPhaseCorr[index2] = new TH1F(Form("h1_deltaT_posPhaseCorr_%s",labelLR_energyBin.c_str()),"",2000,-12000.,12000.);
	        
	      }
	    
	    if (fabs(deltaT - posCorr)<10000 ) {
	      h1_deltaT_posPhaseCorr[index2] -> Fill( deltaT  - posCorr - t1fineCorr );
	    }
	  ...
	  for(int iEnergyBin = 1; iEnergyBin <= nEnergyBins; ++iEnergyBin)
	    {
	      ...
	      if (useTrackInfo) {
                ...
                // -- pos and phase corr deltaT
	        if (!h1_deltaT_posPhaseCorr[index2]) continue;
	        
	        c = new TCanvas(Form("c_deltaT_posPhaseCorr_%s",labelLR_energyBin.c_str()),Form("c_deltaT_posPhaseCorr_%s",labelLR_energyBin.c_str()));
	        
                histo = h1_deltaT_posPhaseCorr[index2];
	        histo -> SetLineWidth(2);
	        histo -> SetLineColor(kGreen+1);
	        histo -> SetMarkerColor(kGreen+1);
	        
                //std::cout << "# Entries in pos-corr phase corr deltaT for bar " << labelLR_energyBin.c_str() << " is " <<  histo -> GetEntries() << std::endl;
	        fitFunc = new TF1(Form("fitFunc_posPhaseCorr_%s",labelLR_energyBin.c_str()),"gaus",-10000, 10000);
	        drawDeltaT(c, histo, fitFunc, "phase-corrected", "ph.Corr", "");      	      
	        ...
	        histo -> SetMarkerColor(kBlue);
                ...
                //std::cout << "# Entries in pos-corr deltaT for bar " << labelLR_energyBin.c_str() << " is " <<  histo -> GetEntries() << std::endl;
	        fitFunc = new TF1(Form("fitFunc_posCorr_%s",labelLR_energyBin.c_str()),"gaus",-10000, 10000);
	        drawDeltaT(c, histo, fitFunc, "pos-corrected", "posCorr", "same"); 
	        
	        
	        c -> Print(Form("%s/CTR_posCorr_phaseCorr/c_deltaT_posPhaseCorr__%s.pdf",plotDir.c_str(),labelLR_energyBin.c_str()));
	        c -> Print(Form("%s/CTR_posCorr_phaseCorr/c_deltaT_posPhaseCorr__%s.png",plotDir.c_str(),labelLR_energyBin.c_str()));
	        delete c;
		...
		fitFunc1_posCorr[index2] -> SetRange( prof -> GetMean()-2*prof->GetRMS(), prof -> GetMean()+2*prof->GetRMS());
  ...
  for(auto mapIt : trees)
      ...
      for(int entry = 0; entry < nEntries; ++entry)
	{
	  ...
	  if (anEvent->nhits < 0) continue;
	  if (anEvent->x<-100) continue;
