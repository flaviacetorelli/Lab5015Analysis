...
int main(int argc, char** argv)
{
  ...
  if( argc < 2 )
  {
    std::cout << ">>> moduleCharacterization_step1_off::usage:   " << argv[0] << " configFile.cfg" << std::endl;
    return -1;
  }
  ...
  // -- read offset for each bar from file
  std::string offsetFileName = opts.GetOpt<std::string>("Input.offsetFileName");
  std::map < std::pair<int, float>, float> offsetL; 
  std::map < std::pair<int, float>, float> offsetR;
  ...
  if (offsetFileName != "NOOFFSET") {
    ...
    while ( offsetFile.good() ){
      ...
      if (side == "L") 
      {
         offsetL[std::make_pair(bar,ov)] = value; 
         std::cout<< bar <<  "   " << ov << "  " << offsetL[std::make_pair(bar,ov)] <<std::endl;
      }
      else{
         offsetR[std::make_pair(bar,ov)] = value; 
         std::cout<< bar <<  "   " << ov << "  " << offsetR[std::make_pair(bar,ov)] <<std::endl;
      }
    ...
    for(unsigned int iBar = 0; iBar < channelMapping.size()/2; ++iBar){
      for(unsigned int ii = 0; ii < Vov.size(); ++ii){
	offsetR[std::make_pair(iBar, Vov[ii])] = 0.;
	offsetL[std::make_pair(iBar, Vov[ii])] = 0.;
      }
    }
  }
  ...
  for(int entry = 0; entry < nEntries; ++entry) {
	...
	if( h1_totL[index] == NULL )
	{
	  ...
	  h1_totL[index] = new TH1F(Form("h1_tot_bar%02dL_Vov%.2f_th%02.0f",iBar,Vov,vth),"",500,-5.,5.); // -10, 40  before
	  h1_totR[index] = new TH1F(Form("h1_tot_bar%02dR_Vov%.2f_th%02.0f",iBar,Vov,vth),"",500,-5.,5.);
	  ...
	  h1_energyLR[index] = new TH1F(Form("h1_energy_bar%02dL-R_Vov%.2f_th%02.0f",iBar,Vov,vth),"",map_energyBins[Vov],map_energyMins[Vov],map_energyMaxs[Vov]);
        ...
        //std::cout << "tot: " << totL[maxBar] << " offset: " << offsetL[std::make_pair(maxBar, Vov)] << " corrected: " << totL[maxBar] - offsetL[std::make_pair(maxBar, Vov)] << std::endl;
	...
	if( !opts.GetOpt<std::string>("Input.sourceName").compare("Co60") ||
	  ...
	  h1_totL[index] -> Fill( totL[iBar] - offsetL[std::make_pair(iBar, Vov)]  );
	  h1_energyL[index] -> Fill( energyL[iBar] );

	  h1_qfineR[index] -> Fill( qfineR[iBar] );
	  h1_totR[index] -> Fill( totR[iBar] - offsetR[std::make_pair(iBar, Vov)] );
	  ...
	  anEvent.totL = totL[iBar] - offsetL[std::make_pair(iBar, Vov)];
	  anEvent.totR = totR[iBar] - offsetR[std::make_pair(iBar, Vov)];
