...
int main(int argc, char** argv)
{
  ...
  for(int entry = 0; entry < nEntries; ++entry) {
	...
	if( h1_totL[index] == NULL )
	{
	  ...
	  h1_totL[index] = new TH1F(Form("h1_tot_bar%02dL_Vov%.2f_th%02.0f",iBar,Vov,vth),"",500,-2.,2.); // -10, 40  before
	  h1_totR[index] = new TH1F(Form("h1_tot_bar%02dR_Vov%.2f_th%02.0f",iBar,Vov,vth),"",500,-2.,2.);
	...
	if( !opts.GetOpt<std::string>("Input.sourceName").compare("Co60") ||
	  ...
	  if( (energyL[iBar] +  energyR[iBar])/2 <  minE[std::make_pair(iBar, Vov)] ) continue; // cut low energy events
